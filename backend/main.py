...
from backend.constants import FACE_MATCH_THRESHOLD, FIGURE_MATCH_THRESHOLD
from backend.figure_detector import FigureDetector
...
def merge_noface_biometrics(event_people, using_facial_data = False, testing_dicts=None) -> list[Person]:
    ...
    def calc_dist(person_num, person, other_person_num, other_person, collected_data):
        ...
        new_person = Person(person.appearances[:])  # make a copy
        new_person.face_image = person.face_image
    ...
    while not done:
        ...
        if smallest_dist < FIGURE_MATCH_THRESHOLD:
            ...
            if i < j:
                ...
                if(event_people[i].face_image is None and event_people[j].face_image is not None):
                    event_people[i].face_image = event_people[j].face_image
                event_people.pop(j)
            else:
                event_people[j].appearances.extend(event_people[i].appearances)
                if(event_people[j].face_image is None and event_people[i].face_image is not None):
                    event_people[j].face_image = event_people[i].face_image
                event_people.pop(i)
...
def encode_image(np_img: np.ndarray) -> str:
...
@app.get("/data")
def get_data(use_face: Optional[bool] = True, use_clothes: Optional[bool] = True):

    print("using face data: " + str(use_face))
    print("using clothing data: " + str(use_clothes))

    figure_detector = FigureDetector()

    event_image_names = os.listdir("backend/runtime_temp/event")


    if os.path.isdir("backend/runtime_temp/figures"):
        shutil.rmtree("backend/runtime_temp/figures")

    event_people: list[Appearance] = []


    for photo_num, event_image_name in enumerate(event_image_names):
        figures = figure_detector.get_figures_from_photo("backend/runtime_temp/event/" + event_image_name)
    ...
    if use_clothes:
        merge_noface_biometrics(event_people, use_face)

    
    return_obs = [
            {
                "description": person.get_dict(),
                "face": (encode_image(person.face_image) if person.face_image is not None and person.face_image.size != 0 else ""),
                "appearances": 
                [
                    {
                        "photo_num": a.photo_num,
                        "image": encode_image(a.figure.image)
                    }
...
upload_dir = "backend/runtime_temp/event"
...
@app.get("/images")
def list_images():
    files = os.listdir("backend/runtime_temp/event")

    return [f"http://localhost:8000/static/{file}" for file in files]
