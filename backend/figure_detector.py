...
from backend.ColourSelection import get_detailed_colour_kmeans, luminance
from backend.constants import FIGURE_DETECTION_THRESHOLD, FIGURE_PIXEL_QUANTITY_THRESHOLD, MIN_MASK_SIZE
from backend.face_data import FaceData
from backend.clothing_detector import ClothingDetector
from backend.face_detector import FaceDetector
from backend.feature import Feature
from backend.figure import Figure
   ...
   # Reset detector between events for testing
    ...
    def detect_figures(self, image: np.ndarray, display_bound=False) -> list[tuple[tuple[int, int], tuple[int, int]]]:
        ...
        for i, mask in enumerate(results[0].masks.data):
            ...
            black = (person_segment[:, :, 0] == 0) & (person_segment[:, :, 1] == 0) & (person_segment[:, :, 2] == 0)
            person_segment[black] = [227, 227, 227]
